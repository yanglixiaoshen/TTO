...
def process_config(config):
    ...
    media_root = '/media/yl/yl_8t/'
    # create some important directories to be used for that experiments
    config.summary_dir = os.path.join(media_root, 'traTransformer_experiments', 'tb', exp_name)
    config.checkpoint_dir = os.path.join(media_root, 'traTransformer_experiments', 'save', exp_name, 'checkpoints/')
    config.result_dir = os.path.join(media_root, 'traTransformer_experiments', 'save', exp_name, 'results/')
...
class MetricTracker:
    def __init__(self, *keys, writer=None):
        self.writer = writer
        self._data = pd.DataFrame(index=keys, columns=['total', 'counts', 'average'])   # row: 'loss', 'acc1', 'acc5'
        self.reset()
...
if __name__ == '__main__':
    ensure_dir('/media/yl/yl_8t/exp1/vis')